...
_G.handleNewID = function(id, infoType)
	...
	if itemName == "Unknown" or itemName == "" then
		return
	end
	foundIDs[id] = true
...
FinderTab:CreateLabel(
	"If the search finds nothing, this game likely uses server-side checks or has no exploitable purchase IDs."
)
FinderTab:CreateLabel(
	"⚠️ Note: Don't rely only on the search! Try to buy something from the shop manually; sometimes the signal triggers anyway."
)
...
local function refreshPlayerList()
	...
	pcall(function()
		PlayerDropdown:Refresh(#pList > 0 and pList or { "No players found" }, true)
	end)
end
...
-- 5. INFO TAB
InfoTab:CreateSection("Universal Fake Purchase Tool")

InfoTab:CreateParagraph({
	Title = "About",
	Content =
		"This tool searches for Roblox purchase IDs and allows testing of fake purchase signals through MarketplaceService listeners."
})

InfoTab:CreateSection("Features")

InfoTab:CreateLabel("• Deep Scan Finder")
InfoTab:CreateLabel("• Dynamic Quick Actions")
InfoTab:CreateLabel("• Manual Product Execution")
InfoTab:CreateLabel("• Targeted Player Execution")
InfoTab:CreateLabel("• Spam Signals")
InfoTab:CreateLabel("• Auto Search")
InfoTab:CreateLabel("• Marketplace Listeners")
...
InfoTab:CreateParagraph({
	Title = "How To Use",
	Content =
		"1. Scan the game using Finder.\n2. Select or copy an ID.\n3. Use Actions or Players tab.\n4. Execute manually or targeted.\n5. Some games may use server-side protection."
})

InfoTab:CreateSection("Credits")

InfoTab:CreateLabel("UI Library: Rayfield")
InfoTab:CreateLabel("Made by Pixel Core")
